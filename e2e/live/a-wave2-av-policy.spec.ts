...
test.describe("Live Wave 2 A/V policy", () => {
  ...
  test("Raise hand → grant → revoke; unauthorized student denied", async ({
    ...
    // Prefer drawer controls — strip Grant can sit under the drawer and get pointer-blocked.
    const drawerGrant = teacherPage.locator(".meet-people [data-testid='live-av-grant']").first();
    await expect(drawerGrant).toBeVisible({ timeout: 15_000 });
    await drawerGrant.click();
    ...
    const drawerRevoke = teacherPage.locator(".meet-people [data-testid='live-av-revoke']").first();
    await expect(drawerRevoke).toBeVisible({ timeout: 10_000 });
    await drawerRevoke.click();
